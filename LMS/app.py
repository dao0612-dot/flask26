...
# templates : 동적 파일을 모아 놓음 (crud화면, 레이아웃, index 등....)
import os
...
app = Flask(__name__)
app.secret_key = 'oihfagnaofihaeoin'
#세션을 사용하기 위한 보안키 설정 (아무 문자열이나 입력)
...
def login():
    ...
    # login.html에서 넘어온 폼 데이터 uid / upw
    # 딕셔너리의 get과 동일
    ...
    try: # 예외발생 가능성 있음
        with conn.cursor() as cursor: # db에서 커서객체 사용
            # 1회원 정보 조회
            ...
            if user:
                ...
                # 세션에 저장 완료
                # 브라우저에서 f12번을 누르고 에플리케이션 탭에서 쿠기 항목에 가면 session객체가 보임
                # 이것을 삭제하면 로그아웃 처리됨
...
def join(): # http://localhost:5000/ get 메서드(화면 출력용) post(화면폼처리용)
    ...
    except Exception as e: # 예외발생 시 실행문
        print(f"회원가입 에러: {e}")
        print("가입 중 오류가 발생했습니다.\njoin() 메서드를 확인하세요!!!")
...
@app.route("/mypage") #http://localhost:5000/mypage get 요청시 처리됨
def mypage():
    if 'user_id' not in session: #로그인 상태인지 확인
        return redirect(url_for('login')) #로그인이 아니면 http://local:5000/login으로 보냄
...
def board_write():
    # 1. 사용자가 '글쓰기' 버턴을 눌러서 들어왔을 때 (화면 보여주기)
    if request.method == 'GET':
...
# 1. 게시판 목록 조회
@app.route("/board") # http://localhost/board
def board_list():
    conn = Session.get_connection()
    try:
        with conn.cursor() as cursor:
...
@app.route("/board/view/<int:board_id>") # http://localhost:5000/board/view/99 < 게시물 번호
...
# 5. 파일명 중복 방지용 코드 활용
# 6. db에서 부모객체가 삭제되면 자식 객체를 삭제 되게 cascade 처리함!
...
def filesboard_write():
    ...
    if request.method == 'POST':
        ...
        # 파일 처리시 html 필수 코드 > enctype = "multipart/form-data"
